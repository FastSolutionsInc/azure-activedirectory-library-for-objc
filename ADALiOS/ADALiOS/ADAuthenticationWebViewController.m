...
#pragma unused(navigationType)
    ...
    if ( [[requestURL lowercaseString] hasPrefix:[_endURL lowercaseString]] )
    {
        ...
        dispatch_async( dispatch_get_main_queue(), ^{ [_delegate webAuthenticationDidCompleteWithURL:navigationAction.request.URL]; } );
...
- (void)failWithTimeoutNavigation:(null_unspecified WKNavigation *)navigation {
    AD_LOG_ERROR(@"Request load timeout", NSURLErrorTimedOut, nil);
    [self webView:_webView didFailNavigation:navigation withError:[NSError errorWithDomain:NSURLErrorDomain code:NSURLErrorTimedOut userInfo:nil]];
}
